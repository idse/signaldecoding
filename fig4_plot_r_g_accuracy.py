...
for mode in [0,1,2,3,4]:
    ...
    N_run = ratios.shape[0]
    N_run1 = ratios1.shape[0]
    
    plt.figure()
    
    indices = ~np.any(ratios[:,:,mode]==0,axis=1)
    plt.errorbar(np.mean(ratios[indices,:,mode],axis=0),np.arange(data.N_genes),xerr=np.sqrt(np.var(ratios[indices,:,mode],axis=0)),fmt='o',color='k',capsize=3,ms=3,clip_on=False)
    
    indices = ~np.all(ratios1[:,:,mode]==0,axis=1)
    plt.errorbar(np.mean(ratios1[indices,:,mode],axis=0),np.arange(data.N_genes),xerr=np.sqrt(np.var(ratios1[indices,:,mode],axis=0)),fmt='s',color='r',capsize=3,ms=3,clip_on=False)
    
    """
    for kg in range(data.N_genes):
        plt.scatter(ratios[:,kg,mode],kg*np.ones(N_run),s=5,color='k',zorder=1)
    for kg in range(data.N_genes):
        plt.scatter(ratios1[:,kg,mode],kg*np.ones(N_run1),color='r',zorder=-1)#,xerr=np.sqrt(np.var(ratios1[indices,:,mode],axis=0)),fmt='s',color='r',capsize=3,ms=3)
    """
    ...
    plt.savefig(subdirectory_plot_fig + "/" + 'r_g_fig_posneg_' + name + '_' + cond + file_suffix)
