...
if not os.path.exists(subdirectory_plot_data_reg):
    os.mkdir(subdirectory_plot_data_reg)
...
for cond in conditions:
    ...
    thresh = 1
    ratios = np.zeros((N_run,data.N_genes,2))
    ratios_conf = np.zeros((N_run,data.N_genes,2))
    accuracy = np.zeros((N_run,data.N_genes))
    corrs = np.zeros((N_run,data.N_genes))
    variances = np.zeros((N_run,data.N_genes))
    ...
    for it in range(N_run):
        ...
        f = open(subdirectory_plot_data_reg+'/data_regression_' + mode_reg + kernel + '_' + cond + '_run' + str(it) + ".pickle",'wb')
        ...
        for kg in range(0,data.N_genes):
            ...
            ratios_conf[it,kg,0] = fns_plot.diff_zero(TN,(TN+FN))
            ratios_conf[it,kg,1] = fns_plot.diff_zero(TP,(TP+FP))
            
            accuracy[it,kg] = fns_plot.diff_zero((TP+TN),(TN + FP + FN + TP))
            
            corrs[it,kg],_ = fns_plot.calc_pearsoncorr(target_predict[:,kg], tar_test[:,kg])

            variances[it,kg] = fns_plot.calc_explained_variance(tar_test[:,kg],target_predict[:,kg])
    ...
    pickle.dump((ratios,ratios_conf,accuracy,corrs,variances), f)
    f.close()
