...
elif mode == 'SI_conditions_within':
    condition_train = 'B50'
    subdirectory_plot = directory + '/analysis_regression_sg_multi_z' + str(mode_z) + '_3x10_v2'
    subdirectory_plot_fig = subdirectory_plot
elif mode == 'SI_s_r_g':
    condition_train = 'B50'
    subdirectory_plot = directory + '/analysis_regression_s_r_g_z' + str(mode_z)
    subdirectory_plot_fig = directory + '/SI_fig_s_r_g'
    ...
    for mode_sim in [0,1]:
        ...
        for m in range(N_fates):
            ...
            if mode_sim:
                ...
                indices = markers_colony_thresh==m
            plt.scatter(xx_colony[:],yy_colony[:],color='lightgrey',s=3)
            plt.scatter(xx_colony[indices],yy_colony[indices],color=fns_plot.return_colmaps('fates')[m],s=3)
