...
ID = 20
directory = 'data_expt_' + str(ID) + '_scaled_norm_bgsub_withSmad2'
...
#signal_names = ['ncSMAD23','ncSMAD23','ncSMAD2','pSMAD1','ncYAP','BCat','LEF1','cytopERK']#,'cytopAKT']
#signal_names_simplified = ['SMAD23','SMAD23','SMAD2','SMAD1','YAP','BCat','LEF1','ERK']#,'AKT']
...
if N_markers>0:
    ...
    print(fate_names)
    fate_names_ordered = ['AMLC','PGCLC','PSLC','ecto','meso','pluri','endo','mitotic','none']
    fate_names_save = ['AMLC','PGCLC','PSLC','ecto','meso','pluri','junk']
else:
...
for it in range(0,N_entries):
    ...
    for it_gene in range(0,N_genes):
        if it_gene == 14: #NANOG
            G[ind2,ind3,it_gene] = data[gene_names[it_gene]][it]/1.15
        elif it_gene == 13: #OCT4
            G[ind2,ind3,it_gene] = data[gene_names[it_gene]][it]/1.5
        else:
