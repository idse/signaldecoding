...
def return_colmaps(mode,N_var=None):
    ...
    if mode == 'conditions':
        ...
        # ['amnion','PGCLC','PS','meso','pluri','ecto','endo',other']
        colors = np.array([[90/255,166/255,71/255,1],[227/255,143/255,52/255,1],
                 [211/255,62/255,43/255,1], [140/255,40/255,93/255,1],
                 [75/255,167/255,158/255,1], [49/255,118/255,181/255,1], [227/255,179/255,61/255,1], [0.8,0.8,0.8,1]])
    elif mode == 'signals':
        if N_var==None:
            N_var = 7
        cmap = mpl.cm.get_cmap('tab10')
...
def return_fates(G,gene_names,thresh=1):
    ...
    contains_TBX6 = 'TBX6' in gene_names
    if contains_TBX6:
    ...
    return markers_colony,fate_names
...
def test_train_split_colonies(data,feature,target,train_size=3):
    ...
    #use first N=train_size colonies for testing, so colony 1 can be used for plotting
    ...
    #clean train data
    feat_train,tar_train,metricdist_train,markers_train = clean_data_full_v2(feature_train,target_train,metricdist_train,markers_train)
    
    #clean test data
    feat_test,tar_test,metricdist_test,markers_test = clean_data_full_v2(feature_test,target_test,metricdist_test,markers_test)
...
def test_train_split_cells(data,feature,target,train_size=0.5):
...
def test_train_split_new(data,feature,target,train_size=3):
    if train_size<1:
        return test_train_split_cells(data,feature,target,train_size=train_size)
    else:
        return test_train_split_colonies(data,feature,target,train_size=train_size)
